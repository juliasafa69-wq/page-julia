  ...
  try {
    if (post.liked) {
      ...
      const { error } = await supabaseClient.from("likes").insert([
        {
          post_id: postId,
          visitor_fingerprint: visitorFingerprint || generateVisitorFingerprint(),
          user_name: profileData.name || "Visitante",
          created_at: new Date().toISOString(),
        },
      ]);
      ...
      const { error } = await supabaseClient
        ...
        .delete()
        .eq("post_id", postId)
        .eq("visitor_fingerprint", visitorFingerprint || generateVisitorFingerprint());
