...
async function handleRealtimeMessage(message) {
  ...
  if (chatId) {
    ...
    if (messagesDiv) {
      ...
      const isFromJulia = message.sender_name && message.sender_name.includes("Julia");
...
async function loadUserChatHistory(chatId) {
  try {
    ...
    data?.forEach((msg) => {
      ...
      const messageHTML = `
        <div class="chat-message ${isFromMe ? "sent" : "received"} ${isFromJulia ? "julia-message" : ""}">
...
function openChatWith(friendId, friendName) {
  ...
  if (conversation) {
    ...
    conversation.messages.forEach((msg) => {
      ...
      const messageHTML = `
        <div class="chat-message ${isFromMe ? "sent julia-message" : "received"}">
          <div class="message-text">${msg.text}</div>
          <div class="message-time">${new Date(msg.timestamp).toLocaleTimeString("pt-BR", {
            hour: "2-digit",
            minute: "2-digit",
          })}</div>
        </div>
