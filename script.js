...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  document.addEventListener("click", function (e) {
    ...
    // Não fechar se clicar no ícone de mensagens (header-icon com toggleMessages), dentro do dropdown, ou no botão de deletar
    ...
    const clickedDeleteBtn = e.target.closest(".delete-conversation-btn");

    if (!clickedMessagesIcon && !clickedInsideDropdown && !clickedDeleteBtn) {
          ...
          conv.friendName
                ...
                ${
                  conv.unread > 0
                    ...
                    ${
                      isOwner
                        ? `<button class="delete-conversation-btn" data-friend-id="${conv.friendId}" data-friend-name="${conv.friendName}" style="background: #e74c3c; color: white; border: none; border-radius: 50%; width: 20px; height: 20px; cursor: pointer; font-size: 10px; margin-top: 5px;" title="Deletar conversa">🗑️</button>`
                        : ""
                    }
                </div>
            </div>
...
function setupMessagesListDelegation() {
  ...
  messagesList.addEventListener("click", (e) => {
    ...
    if (e.target.closest(".delete-conversation-btn")) {
      ...
      // Impedir que o evento chege ao listener global do dropdown
      ...
      // Pequeno delay para garantir que o evento foi completamente parado
      setTimeout(() => {
        if (
          confirm(
...
// Mostrar opções do chat (menu flutuante)
function showChatOptions(chatId, friendId, friendName) {
  ...
  menu.innerHTML = `
    <div style="padding: 8px 0;">
      <button onclick="deleteConversationFromChat('${chatId}', '${friendId}', '${friendName.replace(
    /'/g,
    "\\'"
  )}')" style="width: 100%; padding: 10px 15px; border: none; background: none; text-align: left; cursor: pointer; color: #e74c3c; font-size: 14px;">
        <i class="fas fa-eraser" style="margin-right: 8px;"></i>Limpar conversa
      </button>
    </div>
  `;
  ...
  if (chatWidget) {
    ...
    document.addEventListener("click", function closeMenu(e) {
      if (!menu.contains(e.target) && !e.target.closest(".fa-ellipsis-v")) {
        menu.remove();
...
// Limpar conversa (apagar todas as mensagens, mas manter o chat)
async function clearConversation(chatId, friendId, friendName) {
  if (
    confirm(
      `Tem certeza que deseja limpar todas as mensagens da conversa com ${friendName}?`
    )
  ) {
    try {
      // Deletar mensagens dessa conversa (mas não a conversa inteira)
      const { error } = await supabaseClient
        .from("messages")
        .delete()
      ...
      console.log("✅ Mensagens da conversa limpas");

      // Limpar mensagens da interface
      const messagesDiv = document.getElementById(`${chatId}-messages`);
      if (messagesDiv) {
        messagesDiv.innerHTML =
          '<div style="text-align: center; color: #65676b; padding: 20px;">Conversa limpa</div>';
      }
      ...
      alert(`✅ Conversa com ${friendName} foi limpa!`);
    } catch (error) {
...
// Modo de deletar mensagens individuais
let deleteModeActive = false;
let selectedMessages = new Set();

function toggleDeleteMode(chatId) {
  ...
  if (deleteModeActive) {
    ...
    chatMessages.classList.add("delete-mode");
    selectedMessages.clear();

    // Adicionar header de seleção
    const header = document.createElement("div");
    header.id = "delete-mode-header";
    ...
    `;
    header.innerHTML = `
      <span>Selecione as mensagens para apagar</span>
      <button onclick="confirmDeleteSelected('${chatId}')" style="margin-left: 10px; background: #c0392b; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer;">Apagar (${selectedMessages.size})</button>
      <button onclick="toggleDeleteMode('${chatId}')" style="margin-left: 5px; background: #95a5a6; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer;">Cancelar</button>
    `;

    chatMessages.insertBefore(header, chatMessages.firstChild);

    // Adicionar checkboxes às mensagens
    ...
    chatMessages.classList.remove("delete-mode");
    selectedMessages.clear();

    // Remover header
    const header = document.getElementById("delete-mode-header");
...
function updateDeleteHeader(chatId) {
  const header = document.getElementById("delete-mode-header");
  if (header) {
    const button = header.querySelector("button");
    button.textContent = `Apagar (${selectedMessages.size})`;
  }
}

async function confirmDeleteSelected(chatId) {
  ...
  if (
    confirm(
      `Tem certeza que deseja apagar ${selectedMessages.size} mensagem(ns)?`
    )
  ) {
    try {
      // Por enquanto, apenas remove da interface (implementar deleção do banco depois)
      const chatMessages = document.getElementById(`${chatId}-messages`);
      const checkboxes = chatMessages.querySelectorAll(
        ".message-checkbox:checked"
      );

      checkboxes.forEach((checkbox) => {
        const messageElement = checkbox.closest(".chat-message");
        if (messageElement) {
          messageElement.remove();
        }
      });

      alert(`✅ ${selectedMessages.size} mensagem(ns) apagada(s)!`);

      // Desativar modo de deletar
      toggleDeleteMode(chatId);
    } catch (error) {
...
// Deletar conversa do chat (fecha o chat e deleta)
function deleteConversationFromChat(chatId, friendId, friendName) {
  if (
    confirm(
      `Tem certeza que deseja deletar a conversa com ${friendName}? Esta ação não pode ser desfeita.`
    )
  ) {
    // Fechar o chat primeiro
    closeChat(chatId);
  ...
  const chatHTML = `
    <div class="chat-widget chat-position-${position}" id="${chatId}">
      <div class="chat-header">
        ...
        ${
          isOwner
            ? `<button onclick="showChatOptions('${chatId}', '${friendId}', '${friendName.replace(
                /'/g,
                "'"
              )}')" style="background: none; border: none; color: white; cursor: pointer;" title="Opções">
                <i class="fas fa-ellipsis-v"></i>
        </button>`
            : ""
        }
