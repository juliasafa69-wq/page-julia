...
async function getCurrentUserId() {
  ...
  try {
    ...
    if (error) {
      console.warn("⚠️ Erro ao buscar user_id da tabela users:", error);
...
async function logoutAdmin() {
  ...
  try {
    ...
    try {
      ...
      console.warn("⚠️ SignOut local falhou:", e);
    }
    ...
    try {
      ...
      if (session) {
        console.warn("⚠️ Sessão ainda presente, limpando novamente");
        clearSupabaseAuthStorage();
      }
    } catch {}
  } finally {
...
async function loadFriendRequestsFromDB() {
  // 🛡️ PROTEÇÃO: Apenas Julia (owner) pode ver solicitações
  if (!isOwner) {
...
async function loadReelsFromDB() {
  try {
    ...
    if (data && data.length > 0) {
      ...
      console.log("ℹ️ Nenhum reel salvo no banco");
...
// LIMPAR DEFINITIVAMENTE qualquer referência ao ID antigo da Julia
async function deepCleanOldJuliaData() {
  console.log("🧹 EXECUTANDO LIMPEZA PROFUNDA do ID antigo da Julia...");

  const oldJuliaId = '4c1ad2d8-0295-4e0d-9c4f-f175876fd7f9';

try {
  // 1. Deletar TODAS as mensagens antigas do banco
    console.log("📝 Deletando mensagens antigas do banco...");
    const { error: deleteError } = await supabaseClient
      .from("messages")
      .delete()
      .or(`sender_id.eq.${oldJuliaId},recipient_id.eq.${oldJuliaId}`);

    if (deleteError) {
      console.warn("⚠️ Erro ao deletar mensagens antigas:", deleteError);
    } else {
      console.log("✅ Mensagens antigas deletadas do banco");
    }

    // 2. Limpar cache local
    clearOldJuliaData();

    // 3. Limpar variáveis globais se existirem
    if (window.conversations && Array.isArray(window.conversations)) {
      window.conversations = window.conversations.filter(conv =>
        conv.friendId !== oldJuliaId
      );
    }

    if (window.openChats && Array.isArray(window.openChats)) {
      window.openChats = window.openChats.filter(chat =>
        chat.friendId !== oldJuliaId
      );
    }
...
async function loadMessagesFromDB() {
  ...
  // Limpeza profunda antes de carregar (apenas para admin)
  if (isOwner) {
    await deepCleanOldJuliaData();
  }
  ...
  try {
    ...
    if (isOwner) {
    // Admin carrega TODAS as mensagens (enviadas e recebidas)
    // Isso permite que ela veja o histórico completo de conversas
      // query = query.is("recipient_id", null); // REMOVIDO - carregava apenas mensagens recebidas
    } else {
    ...
    // Para admin: validar conversas (apenas verificar existência, sem deletar)
    if (isOwner) {
      const currentUserId = await getCurrentUserId();
      const validConversations = [];

      for (const conv of conversations) {
        // Sempre incluir conversa da própria Julia
        if (conv.friendId === currentUserId) {
          validConversations.push(conv);
          continue;
        }

        // Para outros usuários, verificar se existem
        const userExists = await checkUserExists(conv.friendId);
        if (userExists) {
          validConversations.push(conv);
        } else {
          console.log(`⚠️ Usuário não encontrado, pulando conversa: ${conv.friendId}`);
        }
      }

      conversations = validConversations;
      console.log(`✅ Conversas validadas: ${conversations.length}`);
    }
  ...
  try {
    ...
    if (existingRequests && existingRequests.length > 0) {
      const alreadySentMessage =
        currentLanguage === "es"
          ? "⚠️ Ya enviaste una solicitud pendiente. Espera la respuesta de Julia."
          : "⚠️ Você já enviou uma solicitação pendente. Aguarde a resposta da Julia.";
  ...
  try {
    ...
    if (requestData?.sender_fingerprint) {
      ...
      if (existingFriend) {
        ...
        console.log("⚠️ Duplicidade detectada - visitante já é amigo");
        return;
      }
    }
  } catch (error) {
    // Se der erro na verificação (ex: não encontrou), continua normalmente
    console.log("ℹ️ Verificação de duplicidade pulada:", error.message);
  }
...
async function toggleLike(postId) {
  ...
  console.log(
    `${post.liked ? "❤️ CURTIU" : "💔 DESCURTIU"} - Post ${postId} agora tem ${
      post.likes
    } curtida(s)`
  );
...
function handleAuthFallback(email) {
  alert(
    "⚠️ Estamos teniendo problemas técnicos, pero puedes continuar como visitante 💬\nTus datos se guardarán cuando el sistema vuelva a estar disponible."
  );
...
function showPasswordRecovery() {
  ...
  supabaseClient.auth
    ...
    .catch((error) => {
      ...
      alert(
        "⚠️ Hubo un problema al enviar el correo. Intenta de nuevo más tarde."
      );
    });
}
    ...
    if (file) {
      ...
      try {
        ...
        if (privacyInput === "1") {
          ...
          privacy = "public";
          console.log("⚠️ Privacidade inválida, usando 'public' como padrão");
        }
        ...
        try {
          ...
          if (dbError) {
            ...
            alert(
              "⚠️ Vídeo enviado, mas houve erro ao salvar no banco de dados."
            );
          } else {
...
async function updateProfilePic(imageSrc, positionData = null) {
  try {
    ...
    if (!session) {
      alert("⚠️ Erro de autenticação. Tente fazer login novamente.");
...
async function toggleFollow() {
  // 🛡️ PROTEÇÃO: Bloquear cliques rápidos
...
function applyPrivacySettings() {
  ...
  if (friendsList) {
    // 🛡️ PROTEÇÃO: Esconder lista de amigos para visitantes (mostrar só número)
    if (!isOwner) {
    ...
    try {
      if (currentMedia.type === "photo") {
        ...
        if (index > -1) {
          ...
          if (error) {
            console.warn("⚠️ Erro ao deletar foto do Supabase:", error);
          }
        ...
        if (index > -1) {
          ...
          if (reel.src.includes("supabase.co/storage")) {
            try {
              ...
              if (urlParts.length > 1) {
                ...
                if (storageError) {
                  console.warn(
                    "⚠️ Erro ao deletar vídeo do Storage:",
                    storageError
                  );
                } else {
              ...
              console.warn("⚠️ Erro ao processar delete do Storage:", e);
            }
          }
          ...
          if (dbError) {
            console.warn("⚠️ Erro ao deletar reel do banco:", dbError);
          } else {
...
function setupMessagesListDelegation() {
  ...
  messagesList.addEventListener("click", (e) => {
    ...
    if (e.target.closest(".delete-conversation-btn")) {
      ...
      if (confirm(`Tem certeza que deseja deletar a conversa com ${friendName}? Esta ação não pode ser desfeita.`)) {
    ...
    console.log(
      "🖱️ Clique capturado! Abrindo conversa:",
      friendName,
      "ID:",
      friendId
    );
...
// Marcar mensagens como lidas no banco (apenas para admin)
async function markMessagesAsRead(friendId) {
  try {
    console.log("📖 Marcando mensagens como lidas para:", friendId);

    // Para admin, marcar mensagens dessa conversa como lidas
    // Como admin vê todas as mensagens, marcamos como lidas as enviadas pelo usuário
    const { error } = await supabaseClient
      .from("messages")
      .update({ is_read: true })
    ...
    console.error("❌ Erro ao marcar mensagens como lidas (rede):", error);
    // Não vamos falhar se for erro de rede - as mensagens serão marcadas na próxima vez
  }
}

// Limpar cache de dados antigos
function clearOldJuliaData() {
  const oldJuliaId = '4c1ad2d8-0295-4e0d-9c4f-f175876fd7f9';
  ...
  for (let i = localStorage.length - 1; i >= 0; i--) {
    ...
    if (value && value.includes(oldJuliaId)) {
      console.log('🧹 Removendo localStorage antigo:', key);
      localStorage.removeItem(key);
    }
  }
  ...
  for (let i = sessionStorage.length - 1; i >= 0; i--) {
    ...
    if (value && value.includes(oldJuliaId)) {
      console.log('🧹 Removendo sessionStorage antigo:', key);
      sessionStorage.removeItem(key);
    }
  }
...
async function checkUserExists(userId) {
  try {
    // Se for o ID da própria Julia logada, sempre retorna true
    if (isOwner) {
      const currentUserId = await getCurrentUserId();
...
// Deletar conversa inteira (apenas para admin/owner)
...
async function handleChatEnter(event, chatId, friendId) {
  console.log("⌨️ Tecla pressionada:", event.key, "Chat:", chatId);
...
async function sendMessageFromAdmin(chatId, recipientId) {
  ...
  try {
    ...
    // Verificar se recipientId é um UUID válido (usuário) ou fingerprint (visitante)
    ...
    if (isUUID) {
      ...
      if (userError || !userData) {
        console.warn("⚠️ Usuário não encontrado no banco:", recipientId);
        alert("❌ Este usuário não existe mais no sistema. A conversa está corrompida.");
        return;
      }
      ...
      // É um fingerprint de visitante
      finalRecipientId = null;
      finalFingerprint = recipientId;
    }
    ...
    const messageData = {
      ...
      message_text: message,
      visitor_fingerprint: finalFingerprint,
      is_read: false,
    };

    console.log("📤 Julia enviando:", messageData, "| É UUID?", isUUID);
