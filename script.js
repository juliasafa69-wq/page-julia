...
// Verificar permissões com user fornecido (sem chamar getUser)
async function checkUserPermissionsWithUser(user) {
  console.log("🔍 checkUserPermissionsWithUser chamado com:", user?.id);
  if (!user) {
    console.log("👤 Nenhum usuário fornecido");
    currentUser = null;
    isOwner = false;
    updateInterfaceBasedOnPermissions();
    return;
...
supabaseClient.auth.onAuthStateChange(async (event, session) => {
  ...
  if (event === "SIGNED_IN" && session?.user) {
    ...
    // Verificar permissões após login (usando user do session)
