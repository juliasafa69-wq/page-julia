...
function renderMessagesList() {
  ...
  if (conversations.length === 0) {
    ...
    messagesList.innerHTML = conversations
      .map(
        (conv, index) => `
            <div class="message-item" data-conv-index="${index}">
    ...
    document.querySelectorAll(".message-item").forEach((item) => {
      item.addEventListener("click", () => {
        const index = parseInt(item.getAttribute("data-conv-index"));
        const conv = conversations[index];
        console.log("🖱️ Clicou na conversa:", conv.friendName);
        openChatWith(conv.friendId, conv.friendName);
      });
    });
  }
