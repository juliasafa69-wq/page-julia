...
// LIMPEZA TEMPORARIAMENTE DESABILITADA - CAUSANDO PROBLEMAS
async function deepCleanOldJuliaData() {
  console.log(
    "⚠️ Limpeza profunda TEMPORARIAMENTE desabilitada para diagnóstico"
  );

  // Apenas limpar cache local (sem deletar do banco)
  clearOldJuliaData();

  // Limpar variáveis globais se existirem (sem deletar)
  const oldJuliaId = "4c1ad2d8-0295-4e0d-9c4f-f175876fd7f9";

  if (window.conversations && Array.isArray(window.conversations)) {
    window.conversations = window.conversations.filter(
      (conv) => conv.friendId !== oldJuliaId
    );
  }

  if (window.openChats && Array.isArray(window.openChats)) {
    window.openChats = window.openChats.filter(
      (chat) => chat.friendId !== oldJuliaId
    );
  }

  console.log("✅ Apenas limpeza de cache realizada (sem deletar banco)");
}
    ...
    if (isOwner) {
      ...
      for (const conv of conversations) {
        ...
        if (userExists) {
          ...
          console.log(
            `⚠️ Usuário não encontrado, pulando conversa: ${conv.friendId}`
          );
        }
      }
...
function clearOldJuliaData() {
  const oldJuliaId = '08f687d4-b707-4673-967a-0c3672b5bff1
';
  ...
  try {
    ...
    if (isUUID) {
      ...
      if (userError || !userData) {
        ...
        alert(
          "❌ Este usuário não existe mais no sistema. A conversa está corrompida."
        );
        return;
      }
