...
async function toggleMessages() {
  ...
  if (!user) {
    showAlert("🔒 Faça login para conversar com Julia!", "error");
    return;
  }

  // Usuário logado: abrir chat direto com Julia
...
function openChatWithJulia(user) {
  ...
  if (openChats.length >= 1) {
    showAlert("Você já tem uma conversa aberta com Julia!", "info");
    return;
  }
      ...
      <div class="chat-header">
        <div class="chat-status">
          ...
          <div style="display: flex; align-items: center; gap: 6px;">
            <span class="chat-status-indicator chat-online"></span>
