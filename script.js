...
async function checkUserPermissions() {
  console.log("🔍 INICIANDO checkUserPermissions...");
  try {
    // Timeout para evitar travamento
    const getUserPromise = supabaseClient.auth.getUser();
    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(() => reject(new Error("Timeout em getUser")), 5000)
    );

    const {
      data: { user },
    } = await Promise.race([getUserPromise, timeoutPromise]);
    console.log("👤 User obtido:", user?.id, user?.email);
    ...
    if (user) {
      ...
      try {
        ...
        if (!error && data) {
          ...
          isOwner = false;
          console.log("❌ ERRO ao verificar owner:");
          console.log("   Error:", error);
          console.log("   Data:", data);
          console.log("   Assumindo não-owner");
        }
      } catch (dbError) {
        console.log(
...
supabaseClient.auth.onAuthStateChange(async (event, session) => {
  ...
  if (event === "SIGNED_IN" && session?.user) {
    ...
    // Verificar permissões após login
    await checkUserPermissions();
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  // Verificar status de autenticação e permissões
  await checkUserPermissions();
