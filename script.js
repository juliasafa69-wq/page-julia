...
async function clearConversation(chatId, friendId, friendName) {
  if (confirm(`Tem certeza que deseja limpar todas as mensagens da conversa com ${friendName}?`)) {
    try {
      ...
      if (messagesDiv) {
        messagesDiv.innerHTML = '<div style="text-align: center; color: #65676b; padding: 20px;">Conversa limpa</div>';
      }
...
async function confirmDeleteSelected(chatId) {
  ...
  if (confirm(`Tem certeza que deseja apagar ${selectedMessages.size} mensagem(ns)?`)) {
    try {
