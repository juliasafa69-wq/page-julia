...
async function ensureAuthOrThrow() {
  ...
  throw new Error("É necessário estar autenticado no Supabase para enviar fotos.");
}
...
async function loadFollowersFromDB() {
  try {
    ...
    const isFollowing = data.some(f => f.follower_fingerprint === fingerprint);
...
async function loadPostsFromDB() {
  try {
    ...
    posts = data.map((post) => ({
      ...
      content: post.content || post.text, // Usar content se existir, senão text
      privacy: post.privacy,
      likes: post.likes_count,
      comments: [],
      liked: false,
      image: post.image_url,
      imagePosition: post.image_position ? JSON.parse(post.image_position) : null,
        ...
        (post) => `
          ...
          post.author
                ...
                <div class="post-content">
                    ...
                    ${
                      post.image
                        ? `<img src="${post.image}" alt="Post image" class="post-media" onclick="openPostImageViewer('${post.id}')" style="cursor: pointer; ${post.imagePosition ? `object-position: ${-post.imagePosition.x}px ${-post.imagePosition.y}px;` : ''}">`
                        : ""
                    }
                </div>
...
async function sendFriendRequest() {
  ...
  const defaultName = savedName || (currentLanguage === "es" ? "Visitante" : "Visitante");
  ...
  container.innerHTML = requests
    .map((request) => {
      ...
      const cancelButton = showCancelButton
        ? `<button class="cancel-request-btn" onclick="cancelMyRequest('${request.id}')" style="margin-top: 8px; padding: 6px 12px; background: #ff4d4d; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 12px;">
...
async function handleAuth() {
  ...
  try {
    ...
    if (loginError) {
      ...
      if (loginError.message.includes("Invalid login") || loginError.message.includes("credentials")) {
...
function handleAuthFallback(email) {
  alert("⚠️ Estamos teniendo problemas técnicos, pero puedes continuar como visitante 💬\nTus datos se guardarán cuando el sistema vuelva a estar disponible.");
...
function showPasswordRecovery() {
  const email = prompt("📧 Escribe tu correo electrónico para recuperar tu contraseña:");
  ...
  .catch((error) => {
    ...
    alert("⚠️ Hubo un problema al enviar el correo. Intenta de nuevo más tarde.");
  });
}
...
async function updateProfilePic(imageSrc, positionData = null) {
  try {
    ...
    const { error: dbError } = await supabaseClient.from("profile").update({
