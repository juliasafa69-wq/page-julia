  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      reels = await Promise.all(
        data.map(async (reel) => {
          // Carregar comentários do reel (usando tabela comments genérica)
          const { data: commentsData } = await supabaseClient
            .from("comments")
            .select("*")
          ...
          // Verificar se usuário curtiu (usando tabela likes genérica)
          const { data: likeData } = await supabaseClient
            .from("likes")
            .select("id")
            .eq("reel_id", reel.reel_id)
...
async function toggleLike(postId) {
  ...
  try {
    if (post.liked) {
      ...
      if (error) throw error;
      console.log("✅ Curtida salva no banco");
    } else {
      ...
      if (error) throw error;
      console.log("✅ Curtida removida do banco");
    }
  } catch (error) {
...
async function toggleMediaLike() {
  ...
  try {
    if (currentMedia.type === "reel") {
      if (media.liked) {
        // Adicionar curtida (usando tabela likes genérica)
        const { error } = await supabaseClient.from("likes").insert([
          {
            reel_id: media.id,
            visitor_fingerprint: fingerprint,
        ...
        // Remover curtida (usando tabela likes genérica)
        const { error } = await supabaseClient
          .from("likes")
          .delete()
          .eq("reel_id", media.id)
        ...
        ${
          media.comments && media.comments.length > 0
            ? media.comments
                .map(
                  (c) => `
            <div style="display: flex; gap: 10px; margin-bottom: 15px; padding: 10px; background: #f0f2f5; border-radius: 8px;">
              <img src="${
...
async function submitMediaComment() {
  ...
  const fingerprint = visitorFingerprint || generateVisitorFingerprint();
  const authorName = profileData?.name || "Visitante";

  const newComment = {
    author: authorName,
    authorPic: profileData?.profilePic || "https://picsum.photos/32?random=2",
    text: text,
  };
