...
async function checkAdminAccess() {
  // Prevenir login automático após logout
  if (localStorage.getItem("logout_in_progress") === "true") {
    localStorage.removeItem("logout_in_progress");
    isOwner = false;
    console.log("🔑 Logout detectado - bloqueando login automático");
    return false;
  }
...
async function logoutAdmin() {
  ...
  try {
    ...
    localStorage.setItem("logout_in_progress", "true"); // Prevenir login automático
    isOwner = false;
    currentUser = null;
...
async function createPost() {
  ...
  try {
    ...
    const { data, error } = await supabaseClient
      ...
      .insert([
        {
          sender_uid: user?.id || null, // ID do usuário autenticado ou null
          text: text,
          image_url: imageUrl,
          privacy: privacy,
          likes_count: 0,
