...
// Verificar se usuário é owner (Julia)
async function checkUserPermissions() {
  try {
    ...
    if (user) {
      ...
      // Verificar se é owner (só se estiver autenticado)
      try {
        ...
        if (!error && data) {
          ...
          console.log("✅ Usuário logado como owner:", isOwner);
        } else {
          isOwner = false;
          console.log("👤 Usuário autenticado mas não é owner");
        }
      } catch (dbError) {
        console.log(
          "⚠️ Erro ao verificar permissões no banco, assumindo não-owner"
        );
        isOwner = false;
      }
    } else {
      ...
      console.log("👤 Visitante anônimo. Fingerprint:", visitorFingerprint);
    }
    ...
    // Assumir visitante anônimo em caso de erro
    currentUser = null;
    isOwner = false;
    visitorFingerprint = generateVisitorFingerprint();
    updateInterfaceBasedOnPermissions();
...
function showOwnerInterface() {
  ...
  const photosStat = document.querySelector('.stat[title="Número de fotos (acesso limitado)"]');
  if (photosStat) {
...
function showVisitorInterface() {
  // Esconder área de criar posts
  const createPostDiv = document.querySelector(".create-post");
  if (createPostDiv) {
    createPostDiv.innerHTML = `
            <div class="visitor-info" style="text-align: center; padding: 20px; background: #f8f9fa; border-radius: 8px;">
                <p style="margin: 0 0 10px 0;">💖 holá amores, bienvenidos a todos ! 😘</p>
                <button onclick="showLoginModal()" style="margin-top: 10px; padding: 8px 16px; background: var(--primary-color); color: white; border: none; border-radius: 4px; cursor: pointer;">
                    🔑 É você, Julia? Faça login
                </button>
            </div>
        `;
  }
  ...
  const friendsStat = document.querySelector('.stat[onclick="openFriendsModal()"]');
  if (friendsStat) {
  ...
  const photosStat = document.querySelector('.stat[onclick="openGallery(\'photos\')"]');
  if (photosStat) {
...
function addLogoutButton() {
  ...
  const settingsIcon = document.querySelector('.header-icon[onclick="openSettings()"]');
  if (settingsIcon) {
...
async function loadFriendRequestsFromDB() {
  try {
    ...
    friendRequests = data.map((request) => ({
      ...
      pic: `https://via.placeholder.com/50/cccccc/666666?text=${request.sender_name.charAt(
        0
      )}`,
  ...
  try {
    ...
    posts = data.map((post) => ({
      ...
      authorPic:
        ...
        "https://via.placeholder.com/40/cccccc/666666?text=Foto",
  ...
  try {
    ...
    const commentData = {
      ...
      author_avatar:
        ...
        "https://via.placeholder.com/32/cccccc/666666?text=V",
      visitor_fingerprint: visitorFingerprint,
      visitor_ip: await getVisitorIP(),
    };
...
async function sendMessageToJulia(message, senderName) {
  try {
    ...
    const messageData = {
      ...
      sender_avatar:
        currentUser?.user_metadata?.avatar_url ||
        "https://via.placeholder.com/40/cccccc/666666?text=V",
...
// Executar verificação (sem bloquear acesso)
checkAuthStatus();
...
supabaseClient.auth.onAuthStateChange(async (event, session) => {
  ...
  if (event === "SIGNED_IN" && session?.user) {
    ...
    await setUserOnlineStatus(session.user);
  } else if (event === "SIGNED_OUT") {
    console.log("👋 Usuário deslogado detectado via onAuthStateChange");
    // Status já é atualizado para offline no logoutJulia()
  }
});
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  // Verificar status de autenticação (sem bloquear)
  ...
  setTimeout(() => {
    ...
    if (finalBadge) {
      ...
      console.log(
        "🔍 Todos os spans no header:",
        document.querySelectorAll(".header-icon span")
      );
    }
  }, 2000);
});
...
async function sendFriendRequest() {
  ...
  const userName =
    prompt("Digite seu nome para enviar solicitação:", "Visitante") ||
    "Anônimo";
  ...
  if (authError || !user) {
    alert("❌ Erro ao obter usuário autenticado. Faça login primeiro.");
  ...
  try {
    ...
    friendRequests.push({
      id: requestId,
      name: userName,
      pic: `https://via.placeholder.com/50/cccccc/666666?text=${userName.charAt(
        0
      )}`,
      message: "Podemos ser amigos?",
    });
...
async function acceptFriend(requestId, friendName) {
  ...
  const newFriend = {
    id: requestId,
    name: friendName,
    pic: `https://via.placeholder.com/32/cccccc/666666?text=${friendName.charAt(
      0
    )}`,
...
async function saveSettings() {
  // Verificar se é owner antes de salvar configurações  
  if (!isOwner) {
    alert("❌ Apenas a Julia pode alterar as configurações!");
    return;
  }
...
function openChatWith(friendId, friendName) {
  ...
  if (!isFriend) {
    alert("❌ Você só pode conversar com pessoas que são suas amigas! Envie uma solicitação de amizade primeiro.");
    return;
  }
            ...
            <div class="chat-minimized-header">
                <div class="chat-minimized-info">
                    <div class="chat-minimized-avatar">
                        <img src="${
                          conversation?.friendPic ||
                          "https://via.placeholder.com/28/cccccc/666666?text=" +
                            chatData.friendName.charAt(0)
...
function openSecurityCenter() {
  ...
  if (!isOwner) {
    alert("🛡️ Apenas a Julia (administradora) pode acessar a Central de Segurança!");
    return;
  }
...
function handleChatEnter(event, chatId, friendId) {
  if (event.key === "Enter" && event.target.value.trim()) {
    ...
    if (!isFriend) {
      alert("❌ Você não pode mais enviar mensagens para esta pessoa! Vocês não são mais amigos.");
      return;
    }
  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      data.forEach((amigo) => {
        ...
        friendPic.src =
          amigo.friend_pic ||
          "https://via.placeholder.com/40/cccccc/666666?text=" +
            (amigo.friend_name?.charAt(0) || "A");
