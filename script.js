...
// Trocar cor do tema
function changeThemeColor(primary, hover) {
    document.documentElement.style.setProperty('--primary-color', primary);
    document.documentElement.style.setProperty('--primary-hover', hover);
...
// Aceitar amigo
function acceptFriend(requestId, friendName) {
...
function updateProfilePic(imageSrc, positionData = null) {
    profileData.profilePic = imageSrc;
    profileData.profilePicPosition = positionData; // Salvar dados de posicionamento
    ...
    // Atualizar todas as imagens de perfil na página (usando imagem cortada apenas para o perfil)
    document.querySelectorAll('img[src*="placeholder"]').forEach(img => {
        if (img.alt === 'Sua foto' || img.alt === 'Você') {
            img.src = imageSrc;
            // Aplicar posicionamento apenas nas fotos de perfil pequenas
            if (img.classList.contains('profile-pic') || img.classList.contains('profile-pic-40')) {
                if (positionData) {
    ...
    // Adicionar à galeria de fotos (imagem completa)
    ...
    // Criar post automático (com imagem COMPLETA)
    const newPost = {
        id: photoId + 1,
        author: profileData.name,
        authorPic: imageSrc,
        ...
        image: imageSrc, // Imagem completa na postagem
        privacy: 'public',
        likes: 0,
        comments: [],
        liked: false,
...
// Atualizar função de criar post para salvar fotos
function createPost() {
    const text = document.getElementById('postText').value;
    const privacy = document.getElementById('postPrivacy').value;
    const imageFile = document.getElementById('postImage').files[0];
