  ...
  try {
    ...
    if (user) {
      ...
      try {
        ...
        console.log("⚠️ Erro ao verificar permissões no banco, assumindo não-owner");
        isOwner = false;
      }
    } else {
      // Visitante anônimo
  ...
  if (createPostDiv) {
    createPostDiv.innerHTML = `
            <div class="visitor-info" style="text-align: center; padding: 20px; background: #f8f9fa; border-radius: 8px;">
                <p style="margin: 0 0 10px 0;">👋 Você está visualizando a página de <strong>Julia Safa</strong></p>
                <p style="margin: 0; color: #666;">Pode curtir, comentar e conversar, mas não pode criar posts.</p>
...
async function loadProfileDataFromDB() {
  try {
    ...
    if (data) {
      ...
      if (data.online_status !== undefined) profileData.onlineStatus = data.online_status;
...
function initializeLanguageSelection() {
  const currentLang = profileData.language || currentLanguage || 'pt';
...
function initializeStatusSystem() {
  ...
  if (checkbox) {
    ...
    console.log("✅ Checkbox status inicializado:", profileData.showOnlineStatus);
  }
