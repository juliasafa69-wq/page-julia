  ...
  if (createPostDiv) {
    createPostDiv.innerHTML = `
            <div class="visitor-info" style="text-align: center; padding: 20px; background: #f8f9fa; border-radius: 8px;">
                <p style="margin: 0; font-size: 16px; color: #1c1e21;">💖 Holá amores, bienvenidos a todos! 😘</p>
            </div>
        `;
  }
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  updateHeaderFollowersCount(); // Atualizar contador de seguidores no header
});
...
function toggleFollow() {
  ...
  updateInteractionCounts();
  updateHeaderFollowersCount(); // Atualiza contador no header
  localStorage.setItem("profileData", JSON.stringify(profileData));
}
...
// Atualizar contador de seguidores no header
function updateHeaderFollowersCount() {
  const followersElement = document.getElementById("followersCountNumber");
  if (followersElement) {
    followersElement.textContent = formatNumber(profileData.followersCount);
  }
}
