  ...
  try {
    if (post.liked) {
      ...
      if (!isOwner) {
        ...
        await loadNotifications(); // Atualizar contador
      }
    } else {
...
function renderNotifications() {
  ...
  container.innerHTML = notifications
    .map((notif) => {
      ...
      const iconClass = notif.type === "like" ? "like" : "comment";
...
async function handleNotificationClick(notifId) {
  ...
  // Navegar para o post/reel se existir
  if (notif.post_id) {
    // Scroll até o post
    ...
    // Abrir reel
    const reel = reels.find((r) => r.id === notif.reel_id);
    if (reel) {
      currentMedia = reel;
      showMediaComments();
    }
