...
async function loadMessagesFromDB() {
  ...
  try {
    ...
    // Para admin: limpar conversas com usuários que não existem mais
    if (isOwner) {
      const currentUserId = await getCurrentUserId();
      const validConversations = [];

      for (const conv of conversations) {
        // Pular se for conversa da própria Julia (não deletar mensagens dela mesma)
        ...
        if (userExists) {
          ...
          console.log(`🗑️ Removendo conversa corrompida com usuário inexistente: ${conv.friendId}`);
          // Deletar mensagens dessa conversa corrompida
          await supabaseClient
            .from("messages")
            .delete()
            .or(`sender_id.eq.${conv.friendId},recipient_id.eq.${conv.friendId}`);
        }
      }
      conversations = validConversations;
      console.log(`✅ Após limpeza: ${conversations.length} conversas válidas`);
    }
