  ...
  try {
    ...
    posts = await Promise.all(
      data.map(async (post) => {
        ...
        if (isOwner) {
          likeQuery = likeQuery.eq("user_id", "admin_julia");
        } else {
          likeQuery = likeQuery.eq("visitor_fingerprint", visitorFingerprint);
        }
...
async function toggleLike(postId) {
  ...
  const fingerprint = visitorFingerprint || generateVisitorFingerprint();
  const adminUserId = "admin_julia"; // ID fixo para admin
  ...
  try {
    if (post.liked) {
      ...
      const { error } = await supabaseClient.from("likes").insert([
        {
          post_id: postId,
          user_id: isOwner ? adminUserId : null,
          visitor_fingerprint: isOwner ? null : fingerprint,
          created_at: new Date().toISOString(),
        },
      ]);
      ...
      if (isOwner) {
        deleteQuery = deleteQuery.eq("user_id", adminUserId);
      } else {
        deleteQuery = deleteQuery.eq("visitor_fingerprint", fingerprint);
      }
