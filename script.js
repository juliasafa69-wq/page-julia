...
function clearSupabaseAuthStorage() {
  try {
    ...
    console.log("🗑️ Sessão Supabase removida do storage");
  } catch (e) {
...
function generateVisitorFingerprint() {
  // Verificar se já existe no sessionStorage (único por guia/aba)
  let fingerprint = sessionStorage.getItem("visitor_fingerprint");

  if (fingerprint) {
    console.log("🔑 Fingerprint existente:", fingerprint);
    return fingerprint;
  }

  // Gerar novo fingerprint ÚNICO com componente aleatório no início
  ...
  // Colocar random NO INÍCIO para garantir que seja único
  ...
  fingerprint = btoa(fingerprintData).substring(0, 40); // 40 caracteres para incluir parte aleatória

  // Salvar no sessionStorage (persiste durante a sessão da guia)
...
function applySettingsPermissions() {
  if (isOwner) {
    ...
    console.log("⚙️ Configurações completas para administradora");
  } else {
    ...
    posts = await Promise.all(
      data.map(async (post) => {
        ...
        const comments = commentsData
          ? commentsData.map((c) => {
              // Verificar se pode deletar: Julia pode deletar tudo, usuário só pode deletar próprio comentário
              let canDelete = isOwner; // Julia sempre pode

              if (!canDelete && currentUser) {
                // Usuário logado: verificar se é dono do comentário por user_id
                canDelete = c.user_id === currentUser.id;
                console.log(
                  `🔍 Comentário "${c.text.substring(
                    0,
                    20
                  )}..." - User logado: ${currentUser.id} vs Autor: ${
                    c.user_id
                  } = canDelete: ${canDelete}`
                );
              } else if (!canDelete) {
                // Visitante não logado: verificar por fingerprint
                canDelete = c.visitor_fingerprint === visitorFingerprint;
                console.log(
                  `🔍 Comentário "${c.text.substring(
                    0,
                    20
                  )}..." - Fingerprint atual: ${visitorFingerprint} vs Autor: ${
                    c.visitor_fingerprint
                  } = canDelete: ${canDelete}`
                );
              }
              ...
              return {
                ...
                authorPic:
                  c.author_avatar || "https://picsum.photos/32?random=2",
                text: c.text,
...
async function loadPhotosFromDB() {
  try {
    ...
    if (data && data.length > 0) {
      // Gerar fingerprint se necessário
      if (!visitorFingerprint) {
        visitorFingerprint = generateVisitorFingerprint();
      }
        ...
        data.map(async (photo) => {
          ...
          const comments = commentsData
            ? commentsData.map((c) => {
                ...
                if (!canDelete && currentUser) {
                  // Usuário logado: verificar se é dono do comentário
                  canDelete = c.user_id === currentUser.id;
                  console.log(
                    `🔍 [FOTO] Comentário "${c.text.substring(
                      0,
                      20
                    )}..." - User: ${currentUser.id} vs Autor: ${
                      c.user_id
                    } = canDelete: ${canDelete}`
                  );
                } else if (!canDelete) {
                  // Visitante: verificar por fingerprint
                  canDelete = c.visitor_fingerprint === visitorFingerprint;
                  console.log(
                    `🔍 [FOTO] Comentário "${c.text.substring(
                      0,
                      20
                    )}..." - FP atual: ${visitorFingerprint} vs Autor: ${
                      c.visitor_fingerprint
                    } = canDelete: ${canDelete}`
                  );
                }
      ...
      console.log(
        `? ${photos.length} fotos carregadas com comentários (privacidade: ${
          ...
          const comments = commentsData
            ? commentsData.map((c) => {
                ...
                return {
                  ...
                  author: c.author_name || "Visitante",
                  authorPic:
                    c.author_pic || "https://picsum.photos/32?random=2",
                  text: c.text,
                  visitor_fingerprint: c.visitor_fingerprint,
                  user_id: c.user_id,
                  canDelete: canDelete,
...
async function loadMessagesFromDB() {
  ...
  try {
    ...
    if (isOwner) {
      // Admin carrega TODAS as mensagens (recebidas E enviadas)
      const {
        data: { user },
      } = await supabaseClient.auth.getUser();

      if (user) {
        // Buscar: mensagens recebidas (recipient_id = null) OU mensagens enviadas (sender_id = Julia)
        query = query.or(`recipient_id.is.null,sender_id.eq.${user.id}`);
      } else {
        // Fallback: apenas recebidas se não estiver logado
    ...
    const { data, error } = await query.order("created_at", {
      ascending: true, // Ordem cronológica: mais antigas primeiro
    });
    ...
    data?.forEach((msg) => {
      // Identificar quem é o "outro" na conversa
      let key, name, pic;

      if (isOwner && currentUser) {
        // Julia: agrupar por recipient (se Julia enviou) ou sender (se Julia recebeu)
        if (msg.sender_id === currentUser.id) {
          // Julia ENVIOU - agrupar por destinatário
          key = msg.recipient_id || msg.visitor_fingerprint || "unknown";
          // Buscar nome do destinatário (não temos aqui, usar friendId como fallback)
          name = "Usuário"; // Placeholder
          pic = "https://picsum.photos/40?random=3";
        } else {
          // Julia RECEBEU - agrupar por remetente
          key = msg.sender_id || msg.visitor_fingerprint;
          name = msg.sender_name || "Visitante";
          pic = msg.sender_avatar || "https://picsum.photos/40?random=3";
        }
      } else {
        // Usuário normal: agrupar pelo outro participante
        key = msg.sender_id || msg.visitor_fingerprint;
        name = msg.sender_name || "Visitante";
        pic = msg.sender_avatar || "https://picsum.photos/40?random=3";
      }
      ...
      const formattedMsg = {
        from: msg.sender_name || "Visitante",
      ...
      if (!conversationsMap.has(key)) {
        ...
        conv.lastMessage = msg.message_text; // Atualizar última mensagem
        conv.timestamp = new Date(msg.created_at).toLocaleTimeString("pt-BR", {
          hour: "2-digit",
          minute: "2-digit",
        });
...
async function setupRealtimeChat() {
  try {
    console.log("⚡ Configurando chat em tempo real...");
    ...
    realtimeChannel = supabaseClient
      ...
      .subscribe((status) => {
        if (status === "SUBSCRIBED") {
          console.log("✅ Chat em tempo real ativado!");
        } else {
          console.log("⚠️ Status do canal:", status);
        }
      });
  } catch (error) {
...
async function handleRealtimeMessage(message) {
  ...
  console.log("👤 User atual:", user ? user.id : "sem user");
  console.log("👤 Sender da mensagem:", message.sender_id);
  console.log("🔑 isOwner:", isOwner);

  // ⚠️ Ignorar se for mensagem do próprio usuário (já foi adicionada localmente)
  ...
  // 1. Atualizar lista de conversas
  const senderId = message.sender_id || message.visitor_fingerprint;
  const senderName = message.sender_name || "Visitante";

  // 🔍 Para Julia: identificar por recipient OU sender
  let conversationId = senderId;
  if (isOwner && user && message.recipient_id) {
    // Se Julia enviou, agrupar por recipient
    if (message.sender_id === user.id) {
      conversationId = message.recipient_id;
    }
  }

  let conversation = conversations.find((c) => c.friendId === conversationId);
  ...
  if (!conversation) {
    ...
    conversation = {
      ...
      friendPic: message.sender_avatar || "https://picsum.photos/40?random=3",
    ...
    conversation.lastMessage = message.message_text;
    conversation.timestamp = new Date(message.created_at).toLocaleTimeString(
      "pt-BR",
      {
        hour: "2-digit",
        minute: "2-digit",
    ...
    // Incrementar não lidas se não for o remetente
    if (user && message.sender_id !== user.id) {
      conversation.unread++;
    }

    // Mover para o topo (usar conversationId correto)
    conversations = conversations.filter((c) => c.friendId !== conversationId);
    conversations.unshift(conversation);
  }
  ...
  conversation.messages.push({
    ...
    pic: message.sender_avatar || "https://picsum.photos/40?random=3",
  ...
  // 2. Atualizar chat aberto (se existir)
  let chatId = null;

  if (isOwner) {
    // Julia: chat com o remetente
    chatId = `chat-${conversationId}`;
  } else if (user) {
    // Usuário: sempre chat com Julia
    chatId = `chat-julia`;
  }

  console.log("🔍 Chat ID calculado:", chatId);

  if (chatId) {
    const messagesDiv = document.getElementById(`${chatId}-messages`);
    console.log("📦 MessagesDiv encontrado?", !!messagesDiv);

    if (messagesDiv) {
      console.log("💬 Adicionando mensagem ao chat aberto:", chatId);

      // Determinar se é mensagem da Julia
      const isFromJulia =
        message.sender_name && message.sender_name.includes("Julia");
      const isFromMe = user && message.sender_id === user.id;

      const messageHTML = `
        <div class="chat-message ${isFromMe ? "sent" : "received"} ${
        isFromJulia && !isFromMe ? "julia-message" : ""
      } ${isFromMe && isOwner ? "julia-message" : ""}">
      ...
      console.log("✅ Mensagem adicionada ao chat em tempo real!");
    }
  }
}
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  setupDeleteButtonListener(); // Configurar listener global para botões de deletar
...
async function handleCommentEnter(event, postId) {
  if (event.key === "Enter" && event.target.value.trim()) {
    const post = posts.find((p) => p.id === postId);
    if (post) {
      // Obter usuário atual ANTES de criar o comentário
      const {
        data: { user: currentUser },
      } = await supabaseClient.auth.getUser();
      ...
      try {
        ...
        // Adicionar user_id se estiver logado
        if (currentUser) {
          commentData.user_id = currentUser.id;
        }
        ...
        if (error) {
          ...
          if (data && data[0]) {
            ...
            newComment.user_id = data[0].user_id; // Guardar user_id também
          }
...
async function submitMediaComment() {
  ...
  const authorName = isOwner
    ? profileData?.name || "Julia"
    : currentUser?.user_metadata?.name || currentUser?.email || "Visitante";
  ...
  try {
    if (currentMedia.type === "reel") {
      ...
      // Atualizar comentário local com ID do banco
      if (savedComment && savedComment[0]) {
        media.comments[media.comments.length - 1].id = savedComment[0].id;
      }
      ...
      console.log("? Comentário na foto salvo");
          ...
          .insert([
            {
              ...
              type: "comment_photo",
              source_id: media.id.toString(),
              message: `${authorName} comentou na foto: "${text.substring(
                0,
                50
              )}..."`,
              is_read: false,
...
async function deleteMediaComment(index) {
  ...
  try {
    if (currentMedia.type === "reel") {
      // Deletar pelo ID se tiver, senão pelo texto e autor
      if (comment.id) {
        const { error } = await supabaseClient
          .from("reel_comments")
          .delete()
      ...
      console.log("? Comentário do reel deletado");
    } else if (currentMedia.type === "photo") {
      // Deletar comentário de foto
      if (comment.id) {
        const { error } = await supabaseClient
          .from("comments")
          .delete()
      ...
      console.log("? Comentário da foto deletado");
    }
  } catch (error) {
    console.error("? Erro ao deletar comentário:", error);
                ...
                <div style="display: flex; flex-direction: column; align-items: flex-end;">
                    ...
                    ${
                      conv.unread > 0
                        ...
                        ${
                          isOwner
                            ? `<button class="delete-conversation-btn" data-friend-id="${conv.friendId}" data-friend-name="${conv.friendName}" style="background: #e74c3c; color: white; border: none; border-radius: 50%; width: 20px; height: 20px; cursor: pointer; font-size: 10px; margin-top: 5px; pointer-events: auto;" title="Deletar conversa">🗑️</button>`
                            : ""
                        }
                </div>
            </div>
...
function setupMessagesListDelegation() {
  ...
  // Delegação para abrir conversas (exceto botões de deletar)
  messagesList.addEventListener("click", (e) => {
    // IGNORAR se foi clique no botão de deletar
    if (e.target.closest(".delete-conversation-btn")) {
      return; // Deixa o document listener cuidar disso
    }

    // Se não foi no botão de deletar, abrir conversa normalmente
...
// Configurar listener global para botões de deletar (mais confiável)
function setupDeleteButtonListener() {
  if (window.deleteButtonListenerSet) return;
  ...
  document.addEventListener("click", function (e) {
    if (e.target.closest(".delete-conversation-btn")) {
      ...
      const friendId = deleteBtn.dataset.friendId;
      const friendName = deleteBtn.dataset.friendName;

      console.log("🗑️ Botão deletar clicado:", friendName, friendId);

      if (
        confirm(
          `Tem certeza que deseja deletar a conversa com ${friendName}? Esta ação não pode ser desfeita.`
        )
      ) {
        deleteConversation(friendId, friendName);
      }
    }
  });

  window.deleteButtonListenerSet = true;
  console.log("🎯 Listener global para deletar configurado");
}

// Deletar conversa inteira (apenas para admin)
async function deleteConversation(friendId, friendName) {
  console.log("🗑️ Deletando conversa com:", friendName, "ID:", friendId);

  try {
    // Deletar todas as mensagens dessa conversa do banco
    const { error } = await supabaseClient
      .from("messages")
      .delete()
    ...
    console.log("✅ Conversa deletada do banco");

    // Remover da lista local
    conversations = conversations.filter((conv) => conv.friendId !== friendId);

    // Atualizar interface
    renderMessagesList();
    updateMessagesCount();

    alert(`✅ Conversa com ${friendName} deletada com sucesso!`);
  } catch (error) {
...
async function loadUserChatHistory(chatId) {
  try {
    ...
    data?.forEach((msg) => {
      ...
      // Mensagens da Julia recebidas pelo usuário = classe julia-message
      const isFromJulia = !isFromMe && msg.sender_name !== user.email;
      const messageHTML = `
        <div class="chat-message ${isFromMe ? "sent" : "received"} ${
        isFromJulia ? "julia-message" : ""
      }">
...
function openChatWith(friendId, friendName) {
  ...
  if (conversation) {
    ...
    // Carregar mensagens existentes com classe julia-message
    conversation.messages.forEach((msg) => {
      ...
      // Adicionar mensagem com estilo correto
      const messagesDiv = document.getElementById(`${chatId}-messages`);
      if (messagesDiv.innerHTML.includes("Conversa com")) {
        messagesDiv.innerHTML = "";
      }

      const messageHTML = `
        <div class="chat-message ${
          isFromMe ? "sent julia-message" : "received"
        }">
...
async function sendMessageFromAdmin(chatId, recipientId) {
  ...
  try {
    ...
    // Adicionar visualmente no chat (Julia = classe especial)
    const messagesDiv = document.getElementById(`${chatId}-messages`);
    const messageHTML = `
      <div class="chat-message sent julia-message">
        <div class="message-text">${message}</div>
        <div class="message-time">${new Date().toLocaleTimeString("pt-BR", {
          hour: "2-digit",
          minute: "2-digit",
