...
async function checkUserPermissions() {
  try {
    ...
    if (user) {
      ...
      try {
        console.log("🔍 Verificando permissões no banco para:", user.id);
        ...
        if (!error && data && data.is_owner !== undefined) {
          isOwner = data.is_owner === true;
          console.log("✅ Permissão definida do banco - isOwner:", isOwner);
        } else {
          isOwner = false;
          console.log("👤 Usuário não encontrado ou não é owner");
        }
      } catch (dbError) {
        console.log("⚠️ Erro ao verificar permissões no banco:", dbError);
        isOwner = false;
      }
    } else {
      // Visitante anônimo
...
function updateInterfaceBasedOnPermissions() {
  // DEBUG: Mostrar estado atual antes de configurar interface
  console.log("🔍 DEBUG updateInterfaceBasedOnPermissions():");
...
// Função checkAuthStatus() removida - usar apenas checkUserPermissions() para evitar duplicação

// Verificação removida - será feita em initializeData()
...
supabaseClient.auth.onAuthStateChange(async (event, session) => {
  ...
  if (event === "SIGNED_IN" && session?.user) {
    ...
    try {
      ...
      console.log("🔄 Usuário definido, verificando permissões...");
      
      // Verificar permissões (CRÍTICO) - SEM status online que trava
      await checkUserPermissions();
      console.log("🔄 Permissões verificadas, atualizando interface...");
      
      updateInterfaceBasedOnPermissions();
      console.log("🔄 Interface reconfigurada após login");
      
      // Status online por último (opcional, em background)
      setTimeout(() => {
        setUserOnlineStatus(session.user).catch(() => {});
      }, 100);
    } catch (error) {
      console.error("❌ Erro no listener SIGNED_IN:", error);
    }
    ...
    currentUser = null;
    isOwner = false;
    // Reconfigurar interface após logout  
    updateInterfaceBasedOnPermissions();
    console.log("🔄 Interface reconfigurada após logout");
  }
});
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  // FORÇAR IDIOMA ESPANHOL LOGO NO INÍCIO
  currentLanguage = "es";
  localStorage.setItem("language", "es");
  console.log("🇪🇸 FORÇANDO ESPANHOL como idioma padrão");

  // Verificação de auth será feita em initializeData() via checkUserPermissions()
  ...
  // FORÇAR APLICAÇÃO DO IDIOMA ESPANHOL
  setTimeout(() => {
    updateInterface();
    console.log("🇪🇸 Interface atualizada para espanhol");
  }, 500);
...
async function saveSettings() {
  // DEBUG: Verificar estado atual das permissões
