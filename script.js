...
async function acceptFriend(requestId, friendName) {
  ...
  try {
    ...
    // Criar notificação para o visitante informando que foi aceito
    const { error: notifError } = await supabaseClient
      .from("notifications")
      .insert([
        {
          user_id: null,
          visitor_fingerprint: requestData?.sender_fingerprint || null,
          type: "friend_accepted",
          source_id: requestId.toString(),
          message: `✅ Julia aceitou sua solicitação de amizade!\n\n💡 Dica: Faça login para não perder seus dados ao trocar de dispositivo.`,
          is_read: false,
          created_at: new Date().toISOString(),
        },
      ]);
...
async function removeFriend(friendId, event) {
  ...
  alert(`${friend.name} foi removido da sua lista de amigos.`);
}
...
async function loadNotifications() {
  try {
    ...
    // Se for visitante, filtrar por fingerprint
    if (!isOwner && visitorFingerprint) {
      query = query.eq("visitor_fingerprint", visitorFingerprint);
    }
    // Se for admin, carregar todas (user_id null significa pra admin)
    else if (isOwner) {
...
async function toggleMessages() {
  ...
  const isJuliaFriend = friends.some(
    (friend) => friend.id === "julia_admin" || friend.name === "Julia Safa"
  );
